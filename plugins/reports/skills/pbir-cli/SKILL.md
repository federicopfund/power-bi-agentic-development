...
1. **NEVER edit report JSON files directly.** Always use the CLI (`pbir visuals bind`, `pbir set`, etc.) or the Python object model (`pbir script`). The CLI handles field type resolution, schema validation, and structural integrity that raw JSON manipulation bypasses. Editing JSON directly is the #1 source of broken reports.
...
For bulk visual creation, use `pbir script` with `page.create_visual()` (see **`references/add-new-visual.md`**).
...
For complex automation, use `pbir script` or the object model directly. See **`references/object-model-api.md`** for the full Python API.

```yaml
pbir script FILE "path":
  use: run a script file
  example: pbir script my_script.py "Report.Report"

pbir script --execute "CODE" "path":
  use: run inline Python
  example: pbir script --execute "print(context.report.name)" "Report.Report"

pbir script --object-model:
  use: show API docs

pbir script --list-examples:
  use: list built-in script examples
```
...
references/object-model-api.md: Python object model API for pbir script
